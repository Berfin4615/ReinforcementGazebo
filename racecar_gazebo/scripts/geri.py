...
from geri_env import RacecarEnv
...
class ReplayBuffer():
    ...
    def sample(self, n):
        ...
        a_lst = [torch.tensor(x) for x in a_lst]
        s_batch = torch.tensor(s_lst, dtype=torch.float).to(self.dev)
        a_batch = torch.tensor(a_lst, dtype=torch.float).unsqueeze(1).to(self.dev)  # Bu satırda `a` tensörünü uygun şekilde yeniden şekillendiriyoruz
...
class PolicyNetwork(nn.Module):
    def __init__(self, state_dim, action_dim, actor_lr, DEVICE):
        ...
        self.fc_1 = nn.Linear(state_dim, 128)  # state_dim burada 2 olmalı
        ...
        self.LOG_STD_MIN = -20
        self.LOG_STD_MAX = 2
        self.max_action = torch.FloatTensor([90] * action_dim).to(self.dev)
        self.min_action = torch.FloatTensor([-90] * action_dim).to(self.dev)
...
class SAC_Agent:
    def __init__(self):
        self.state_dim = 2  # State boyutu (örneğin, 4)
        self.action_dim = 1  # Action boyutu (örneğin, 2)
        self.lr_pi = 0.0001
        self.lr_q = 0.0005
        self.gamma = 0.98
        self.batch_size = 200
    ...
    def choose_action(self, s):
        with torch.no_grad():
            ...
            action = action.squeeze().cpu().numpy()  # action, numpy dizisi olmadan önce
        return action, log_prob
...
if __name__ == '__main__':
    ...
    for EP in range(EPISODE):
        ...
        while not done:
            ...
            print("EP:{}, Reward:{:.1f}".format(EP, reward), "\n")  
